...
Metadata_table <- "./SIP_metadata.csv"
...
read_delim(Metadata_table,
  ...
  mutate(`Density zone` = factor(ifelse(`Density (g ml-1)` > 1.70, "Heavy", "Light"), levels = c("Light", "Heavy"))) %>% # critical for DESeq2 that the reference is the first level
  identity() ->
  Metadata
...
Ps_obj %>% 
  phyloseq_merge_samples("FractionID", fun = "sum") %>%
...
Ps_obj_merged %>% 
  get_variable() %>% 
  select_if(is.numeric) %>% 
  # add pseudocount before log to avoid -Inf/NaN
  modify(~ log(.x + 1)) %>% 
  # optionally remove columns or rows with NAs
  # e.g., drop columns with any NA:
  discard(~ any(is.na(.x))) %>% 
  vis_value()
...
The frequency analysis detected $`r sum(contamdf_prev$contaminant)`$ sequences as contaminants. In total $`r sum(contamdf_prev$contaminant)`$ were detected as contaminants.
